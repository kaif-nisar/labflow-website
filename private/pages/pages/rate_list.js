...
<td class="commission-rate-cell">
    <input type="number" class="commission-rate" data-test-id="${test._id}" value="${test.commissionToUser }">
</td> <!-- Commission rate input -->
<td class="assigned-price">${(test.finalPrice || test.assignedPriceToUser) - test.basePrice}</td> <!-- Assigned Price (calculated) -->
<td class="franchisee-id">${_id}</td>
<td class="final-price1">${test.finalPrice || test.assignedPriceToUser}</td>`;
    ...
    document.getElementById('saveBtn').addEventListener('click', async function () {
        ...
        try {
            ...
            console.error('Error in save button handler:', error.message);
        }finally {
