(async function ppal() {
    ...
    async function fetchTestList() {
        try {
            ...
            // Check if all responses are success
...
<td class="my-price">${test.myPrice}</td> <!-- Price -->
...
<td class="final-price1">${test.finalPrice || test.assignedPriceToFranchisee}</td>`;
    ...
    document.getElementById('saveBtn').addEventListener('click', async function () {
        ...
        rows.forEach((row, index) => {
            ...
            const franchiseeId = row.querySelector('.franchisee-id').textContent.trim(); // Franchisee ID
    ...
    async function sendDataToBackend(items) {
        // Show the spinner when the data sending starts
        ...
        try {
            ...
            // Hide the spinner after the data has been processed (either success or failure)
